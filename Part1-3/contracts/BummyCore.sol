...
contract BummyCore is BummyMinting {
    ...
    constructor() {
        ...
        _createBummy(0, 0, 0, type(uint256).max,address(0x000000000000000000000000000000000000dEaD));
    }
    ...
    function getBummy(uint256 _id)
        ...
        isGestating = (bum.siringWithId != 0); // 임신기간인지
        isReady = (bum.cooldownEndTime <= block.timestamp); // 교배가능한 상태인지
        cooldownIndex = uint256(bum.cooldownIndex);
        nextActionAt = uint256(bum.cooldownEndTime);
        siringWithId = uint256(bum.siringWithId);
